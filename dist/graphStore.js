...
class GraphStore {
    constructor(baseDir) {
        const dbPath = node_path_1.default.join(baseDir, ".smart-context-map.db");
    ...
    upsertGraph(graph) {
        const tx = this.db.transaction(() => {
            ...
            const insertSymbol = this.db.prepare(`INSERT INTO symbols(id, name, kind, file, signature, exported, loc_start, loc_end)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);
            for (const s of graph.symbols) {
                insertSymbol.run(s.id, s.name, s.kind, s.file, s.signature, s.exported ? 1 : 0, s.locStart, s.locEnd);
            }
            const insertImport = this.db.prepare("INSERT INTO import_edges(from_file, to_file, specifiers) VALUES (?, ?, ?)");
            for (const e of graph.importEdges) {
                insertImport.run(e.fromFile, e.toFile, JSON.stringify(e.specifiers));
            }
            const insertCall = this.db.prepare("INSERT INTO call_edges(caller_symbol_id, callee_name) VALUES (?, ?)");
    ...
    getStats() {
        ...
        const importEdges = this.db.prepare("SELECT COUNT(*) AS c FROM import_edges").get();
        const callEdges = this.db.prepare("SELECT COUNT(*) AS c FROM call_edges").get();
        const meta = this.db
    ...
    getImpactRadius(changedFiles, maxDepth) {
      ...
      `)
            .all(seedJson, maxDepth);
        return [
            ...upstreamRows.map((r) => ({ file: r.file, depth: r.depth, direction: "upstream" })),
            ...downstreamRows.map((r) => ({ file: r.file, depth: r.depth, direction: "downstream" })),
        ];
    }
    getHubFiles(limit) {
        const rows = this.db
    ...
    readGraphData() {
        const files = this.db.prepare("SELECT path FROM files ORDER BY path").all().map((r) => r.path);
      ...
      `)
        ...
        const importEdges = this.db
            ...
            toFile: r.to_file,
            specifiers: JSON.parse(r.specifiers || "[]"),
        }));
