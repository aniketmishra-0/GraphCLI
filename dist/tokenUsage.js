...
exports.renderCliTokenDashboard = renderCliTokenDashboard;
exports.createTokenUsageSnapshot = createTokenUsageSnapshot;
exports.createTokenEfficiencyComparison = createTokenEfficiencyComparison;
exports.renderTokenUsageDashboard = renderTokenUsageDashboard;
...
function progressBar(usagePercent, width = 18) {
    const pct = Math.max(0, Math.min(100, usagePercent));
    const filled = Math.round((pct / 100) * width);
    return `[${"#".repeat(filled)}${"-".repeat(width - filled)}]`;
}
function renderCliTokenDashboard(report) {
    const lines = [];
    ...
    if (report.comparison) {
        ...
        lines.push(`- ${report.comparison.withoutGraph.label}: ${report.comparison.withoutGraph.estimatedTokens} tokens | quality ${report.comparison.withoutGraph.quality}/10`);
        lines.push(`- ${report.comparison.withGraph.label}: ${report.comparison.withGraph.estimatedTokens} tokens | quality ${report.comparison.withGraph.quality}/10`);
    ...
    for (const item of report.prompts) {
        ...
        lines.push(`${padRight(label, 24)} ${padRight(String(item.usedTokens), 8)} ${padRight(String(item.remainingTokens), 8)} ${padRight(`${item.usagePercent}%`, 8)} ${progressBar(item.usagePercent)}`);
    }
    lines.push("=".repeat(75));
    return lines.join("\n");
}
function createTokenUsageSnapshot(label, usedTokens, budgetTokens) {
...
function createTokenEfficiencyComparison(withoutGraphTokens, withGraphTokens, withoutLabel = "Without Graph", withLabel = "With Graph") {
...
function usageClass(usagePercent) {
    if (usagePercent >= 95)
        return "critical";
    if (usagePercent >= 80)
        return "warn";
...
function renderTokenUsageDashboard(report, options) {
