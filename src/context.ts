...
function renderFileSection(
  ...
  reasons: string[],
  symbolNameAllowlist: Set<string> | null,
): string {
  ...
  const symbolLines = allowlistedSymbols
    .map(
      (s) =>
        `- ${s.kind} ${s.name} | exported=${s.exported} | loc=${s.locStart}-${s.locEnd} | sig=${s.signature}`,
    )
    .join("\n");
...
function renderHotPath(hotPath: HotPathScore[]): string {
  const lines = hotPath
...
function rankFiles(
  selection: PrunedContextSelection,
...
export function generateContextMarkdown(
  ...
  for (const file of rankedFiles) {
    ...
    const section = renderFileSection(
      graph,
      file,
      selection.interfaceOnlyFiles.includes(file),
      selection.reasonByFile[file] ?? ["unknown"],
      allowlist,
    );
...
export function generateContextXml(
  graph: GraphData,
  selection: PrunedContextSelection,
  hotPath: HotPathScore[],
  budget: ContextBudget,
  ...
  const files = selection.includedFiles
    ...
    .map((file) => {
      const symbols = graph.symbols
        ...
        .map(
          (s) =>
            `<symbol kind="${escapeXml(s.kind)}" name="${escapeXml(s.name)}" exported="${s.exported}" locStart="${s.locStart}" locEnd="${s.locEnd}">${escapeXml(s.signature)}</symbol>`,
        )
        .join("");
...
export function generateContextPacket(
  ...
  const packet: {
    ...
    f: Array<{
      ...
      r: string[];
      i: string[];
      s: Array<{ n: string; k: string; e: 0 | 1; l: string; sg?: string }>;
    }>;
  } = {
    v: 1,
    p: provider,
  ...
  for (const file of rankedFiles) {
    const symbols = graph.symbols
      .filter((s) => s.file === file)
      .filter((s) => {
        if (!selection.interfaceOnlyFiles.includes(file)) return true;
  ...
  const trimStrategies: Array<() => void> = [
    () => {
  ...
  for (const strategy of trimStrategies) {
    if (estimateTokens(serialized) <= budget.maxTokens) break;
    strategy();
...
export function generateTwoStagePackets(
  graph: GraphData,
  selection: PrunedContextSelection,
  hotPath: HotPathScore[],
  budget: ContextBudget,
  ...
  const stage2 = generateContextPacket(
    graph,
    selection,
    hotPath,
    stage2Budget,
