...
interface SingleStageTokenHints {
  budgetTokens: number;
  packetTokens: number;
}

interface TwoStageTokenHints {
  budgetTokens: number;
  stage1PacketTokens: number;
  stage2PacketTokens: number;
}

function providerGuidance(provider: AiProvider): string {
  if (provider === "claude") {
...
export function buildSingleStagePromptWrapper(
  provider: AiProvider,
  task: string,
  packetPath: string,
  tokenHints?: SingleStageTokenHints,
): string {
...
export function buildTwoStagePromptWrapper(
  provider: AiProvider,
  task: string,
  stage1Path: string,
  stage2Path: string,
  tokenHints?: TwoStageTokenHints,
): string {
